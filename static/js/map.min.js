...
let hotelMarkers = [];
let foodMarkers = [];function focusMapMarker(type, index) {
...
document.querySelectorAll('.map-active').forEach(el => el.classList.remove('map-active'));if (card) {
card.classList.add('map-active');
setTimeout(() => card.classList.remove('map-active'), 4000);
}
}document.addEventListener("DOMContentLoaded", () => {
const TILE_URL = "https:
const TILE_ATTR = '&copy; <a href="https:
...
const name = placeMapEl.dataset.name || "Location";if (!isNaN(lat) && !isNaN(lng)) {
placeMap = L.map("place-map", {
scrollWheelZoom: false,
zoomControl: true,
}).setView([lat, lng], 13);L.tileLayer(TILE_URL, { attribution: TILE_ATTR, maxZoom: 18 }).addTo(placeMap);
...
`<strong>${_escHtml(name)}</strong><br><small>📍 ${lat.toFixed(4)}, ${lng.toFixed(4)}</small><br><a href="https:
...
const popup = `<strong>🏨 ${_escHtml(h.name)}</strong><br><small>${h.type ? _escHtml(h.type.charAt(0).toUpperCase() + h.type.slice(1)) : 'Hotel'}</small>${h.price_range ? '<br>💰 ' + _escHtml(h.price_range) : ''}${h.address ? '<br>📍 ' + _escHtml(h.address) : ''}<br><a href="https:
...
const popup = `<strong>🍛 ${_escHtml(f.name)}</strong><br><small>${_escHtml(f.category || 'Food')}</small>${f.where_to_find ? '<br>📍 ' + _escHtml(f.where_to_find) : ''}<br><a href="https:
...
let userMarker = null;function placeUserMarker(lat, lng) {
if (!placeMap) return;
if (userMarker) {
...
const existingLng = parseFloat(adminMapEl.dataset.lng);const defaultLat = !isNaN(existingLat) ? existingLat : 20.5937;
const defaultLng = !isNaN(existingLng) ? existingLng : 78.9629;
const defaultZoom = !isNaN(existingLat) ? 12 : 5;const map = L.map("admin-map").setView([defaultLat, defaultLng], defaultZoom);
L.tileLayer(TILE_URL, { attribution: TILE_ATTR, maxZoom: 18 }).addTo(map);
setTimeout(() => map.invalidateSize(), 250);const latInput = document.getElementById("lat-input");
const lngInput = document.getElementById("lng-input");let marker = null;
...
if (lngInput) lngInput.value = lng.toFixed(6);if (marker) {
...
const validPlaces = places.filter(p => p.latitude && p.longitude);if (validPlaces.length > 0) {
const map = L.map("district-map", {
scrollWheelZoom: false,
zoomControl: true,
});L.tileLayer(TILE_URL, { attribution: TILE_ATTR, maxZoom: 18 }).addTo(map);const bounds = L.latLngBounds();validPlaces.forEach(p => {
...
.addTo(map)
.bindPopup(popupHtml);bounds.extend([lat, lng]);
}
});if (bounds.isValid()) {
...
const routePoints = [];const map = L.map("itinerary-map", {
scrollWheelZoom: false,
zoomControl: true,
});L.tileLayer(TILE_URL, { attribution: TILE_ATTR, maxZoom: 18 }).addTo(map);const bounds = L.latLngBounds();
let stopCount = 0;const dayKeys = Object.keys(days).sort((a, b) => parseInt(a) - parseInt(b));
...
routePoints.push([lat, lng]);const numberIcon = L.divIcon({
...
}setTimeout(() => map.invalidateSize(), 250);} catch (e) { console.error("Itinerary map error:", e); }
}
}});
function _getResponsivePopupOptions(e){const t='undefined'!=typeof window&&window.innerWidth<480;return{maxWidth:t?240:e||300,autoPanPadding:t?[12,12]:[30,30],closeButton:!0}}
window.HY_setSearchRadius=function(e){window.HY_MAP_INSTANCE&&(console.log('[Map] Updating search radius:',e,'km'),window.HY_RADIUS_CIRCLE&&window.HY_RADIUS_CIRCLE.setRadius(1e3*e))};