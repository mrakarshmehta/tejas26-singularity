(function () {
'use strict';window.SNS = window.SNS || {};
...
const mapId = options.mapId || 'sn-map-canvas';if (options.lat) currentLat = parseFloat(options.lat);
...
maxZoom: 18
}).addTo(activeMap);markersLayer = L.layerGroup().addTo(activeMap);
activeMap.on('moveend', () => {
const center = activeMap.getCenter();
const bounds = activeMap.getBounds();
currentLat = center.lat;
currentLng = center.lng;clearTimeout(fetchTimeout);
...
if (params.bounds) url += `&bounds=${encodeURIComponent(params.bounds)}`;fetch(url)
.then(r => r.json())
.then(data => {
...
const stack = document.getElementById('sn-cards-stack');
if (markersLayer) markersLayer.clearLayers();if (!results || results.length === 0) {
if (stack) {
...
const catMeta = CATEGORY_INFO[item.category_code] || { icon: '📍', label: item.category_label || 'Facility' };
const icon = item.icon || catMeta.icon;
let essentialsHtml = '';
if (item.nearby_essentials && item.nearby_essentials.length > 0) {
const essentialItems = item.nearby_essentials.map(e => `
<a href="${_esc(e.directions_url)}" target="_blank" class="sn-essential-item" title="Click to navigate to ${_esc(e.name)}">
<div class="sn-essential-left">
<span class="sn-essential-icon">${e.icon || '📍'}</span>
<span class="sn-essential-name">${_esc(e.name)}</span>
</div>
<div class="sn-essential-right">
<span class="sn-essential-dist">📍 ${_esc(e.distance_formatted)}</span>
<span>🚶 ${_esc(e.travel_summary)}</span>
</div>
</a>
`).join('');essentialsHtml = `
<div class="sn-essentials-toggle" onclick="SNS.toggleEssentials('${cardId}')">
<span>✨ 10 Nearby Essentials around this location</span>
<span id="${cardId}-arrow">▼</span>
</div>
...
const directionsUrl = item.directions_url || `https:
let actionButtons = `
...
</div><!-- Action Buttons: Navigate, Call, Website, View on Map -->
<div class="sn-card-actions">
${actionButtons}
</div><!-- 10 Nearby Essentials -->
${essentialsHtml}
</div>
`;
...
.catch(err => {
console.warn('Wishlist toggle failed:', rawId, err);
})
.finally(() => {
if (btnEl) {
